...
try:
    ...
    # VDSM not available
    #ERR_MSG='local vdsm(/usr/share/vdsm, /etc/vdsm/vdsm.conf) not found\n'
    #err_file = open("../aproc/shell/RHEV_Gather_err", 'a+')
    ...
    while 1 :
        ...
        if response['status']['code'] != 0:
            ...
            for VM_INFO in response['vmList']:
                ...
                MEM_USE = string.atof(VM_STATE['memUsage']) / 100 * float(VM_INFO['memSize'])
                ...
                TIME_TMP= int(time.time()) - string.atoi(VM_INFO['elapsedTime'])
                ...
                print "VM=%s%d%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%d%s%d%s%0.2f%s%0.2f%s%0.2f%s%0.2f%s%0.2f%s%d%s%s%s%s%s%s%s%s%s%d%s%0.2f%s%0.2f%s%0.2f%s%d%s%0.2f%s%d%s" % (STR_SPLIT1, VM_INDEX, STR_SPLIT2, VM_INFO['vmName'], STR_SPLIT2, VM_INFO['emulatedMachine'], STR_SPLIT2, VM_INFO['vmId'], STR_SPLIT2, VM_INFO['smpCoresPerSocket'], STR_SPLIT2, VM_INFO['memSize'], STR_SPLIT2, string.upper(VM_NWT_MAC), STR_SPLIT2, VM_INFO['bridge'], STR_SPLIT2, VM_BOOT_STATE, STR_SPLIT2, TIME_TMP, STR_SPLIT2, CPU_USED, STR_SPLIT2, CPU_IDLE, STR_SPLIT2, (string.atof(VM_STATE['cpuSys'])) / (string.atof(VM_INFO['smpCoresPerSocket'])), STR_SPLIT2, (string.atof(VM_STATE['cpuUser'])) / (string.atof(VM_INFO['smpCoresPerSocket'])), STR_SPLIT2, string.atof(VM_STATE['memUsage']), STR_SPLIT2, MEM_USE, STR_SPLIT2, string.atof(VM_STATE['rxRate']), STR_SPLIT2, string.atof(VM_STATE['txRate']), STR_SPLIT2, string.atof(VM_STATE['rxDropped']), STR_SPLIT2, string.atof(VM_STATE['txDropped']), STR_SPLIT2, VM_NWT_CNT, STR_SPLIT2, VM_DSK_SIZE, STR_SPLIT2, VM_DSK_AVG_R_RATE, STR_SPLIT2, VM_DSK_AVG_W_RATE, STR_SPLIT2, VM_DSK_CNT, STR_SPLIT2, VM_DRV_SIZE, STR_SPLIT2, VM_DRV_CNT, STR_SPLIT2)
