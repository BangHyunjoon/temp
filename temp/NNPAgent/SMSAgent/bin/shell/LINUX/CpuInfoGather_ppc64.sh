...
if [ -z "$vendor" ] ; then
    vendor=`egrep -i "platform" /proc/cpuinfo | tail -1 | awk -F: '{print $2}'`
    value=`echo $vendor | grep -i pSeries | wc -l`
    if [ $value -eq 1 ]; then
        vendor="IBM"
        type="IBM"
        lscpu  > ../aproc/shell/lscpu_cmd.out
        model=`grep "Model name" ../aproc/shell/lscpu_cmd.out | awk -F: '{print $2}' | awk '{print $1}'`
        speed=`grep clock /proc/cpuinfo | tail -1 | awk -F: '{print $2}'| awk -F'.' '{print $1}'`
        physicalCPUCnt=`grep ^Socket ../aproc/shell/lscpu_cmd.out | awk -F: '{print $2}'`
        allphysicalcpucorecnt=`grep ^Core ../aproc/shell/lscpu_cmd.out | awk -F: '{print $2}'`
        allphysicalcpucorecnt=`expr $allphysicalcpucorecnt \* $physicalCPUCnt `
        if [ $logicalcpucorecnt -gt $allphysicalcpucorecnt ] ; then
            hyperthreading=on
        else
            hyperthreading=off
        fi
