...
RECVRY_SHELL=../bin/shell/HP-UX/recovery/hostname_recovery.sh
RECVRY_HOSTNAME_FILE1=../bin/shell/HP-UX/recovery/hostname_old.dat
RECVRY_HOSTNAME_FILE2=../bin/shell/HP-UX/recovery/hostname_set_file_old.dat
CMD=`echo "" > $FILE_DAT`
COMMAND=`mkdir ../bin/shell/HP-UX/recovery >> $FILE_DAT 2>&1 `
RECVRY_HOSTNAME=$1
if [ $1 ] ; then
    ...
    COMMAND=`echo "    CMD=\\\`./shell/HP-UX/hostname_update.sh \"$1\" >> \\\$FILE_DAT\\\`" >> $RECVRY_SHELL`
    ...
    COMMAND=`grep HOSTNAME= /etc/rc.config.d/netconf > $RECVRY_HOSTNAME_FILE2`
