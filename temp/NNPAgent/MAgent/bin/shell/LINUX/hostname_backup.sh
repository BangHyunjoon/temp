...
RECVRY_SHELL=../bin/shell/LINUX/recovery/hostname_recovery.sh
RECVRY_HOSTNAME_FILE1=../bin/shell/LINUX/recovery/hostname_old.dat
RECVRY_HOSTNAME_FILE2=../bin/shell/LINUX/recovery/hostname_set_file_old.dat
CMD=`echo "" > $FILE_DAT`
...
if [ $1 ] ; then
    ...
    COMMAND=`echo "    CMD=\\\`./shell/LINUX/hostname_update.sh \"$1\" >> \\\$FILE_DAT\\\`" >> $RECVRY_SHELL`
    ...
    COMMAND=`grep HOSTNAME= /etc/sysconfig/network > $RECVRY_HOSTNAME_FILE2`
