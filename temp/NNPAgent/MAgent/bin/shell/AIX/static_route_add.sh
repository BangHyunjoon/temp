...
if [ "$S_ROUTE_IP" ] ; then
    if [ "$S_ROUTE_NM" ] ; then
        if [ "$S_ROUTE_GW" ] ; then
#           if [ "$S_ROUTE_MAC" ] ; then
                TMP_CMD=`\cp -f /etc/rc.net /etc/bak1_rc.net`
                #TMP_CMD=`./shell/AIX/static_route_del.sh $S_ROUTE_IP $S_ROUTE_NM $S_ROUTE_GW $S_ROUTE_MAC;echo $?`
                ...
                if [ "$TMP_CMD" = "0" ] ; then
                    TMP_CMD=`echo $S_ROUTE_ADD >> /etc/rc.net;echo $?`
                    CMD=`echo "Static Route ADD command added(/etc/rc.net) result : $TMP_CMD" >> $FILE_DAT`
                else 
