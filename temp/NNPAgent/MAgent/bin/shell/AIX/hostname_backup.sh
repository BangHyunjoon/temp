...
RECVRY_SHELL=../bin/shell/AIX/recovery/hostname_recovery.sh
RECVRY_HOSTNAME_FILE1=../bin/shell/AIX/recovery/hostname_old.dat
CMD=`echo "" > $FILE_DAT`
COMMAND=`mkdir ../bin/shell/AIX/recovery >> $FILE_DAT 2>&1 `
RECVRY_HOSTNAME=$1
if [ $1 ] ; then
    ...
    COMMAND=`echo "    CMD=\\\`./shell/AIX/hostname_update.sh \"$1\" >> \\\$FILE_DAT\\\`" >> $RECVRY_SHELL`
