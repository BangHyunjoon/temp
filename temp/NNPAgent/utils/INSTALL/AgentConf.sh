...
$LINUX)
        ...
        command=`which systemctl 2> /dev/null | grep -v "no " | wc -l`
...
if [ "$OPT" = "-add" ]; then
        ...
        *)
        ...
        $LINUX)
                ...
                if [ $RCTMP = "atint" ]; then
                    ...
                    if [ -z "$NOMNICHKLIN" -a $SYSTEMCTL -eq 0 ]; then
                        echo "cd" $HOME_PATH"/MAgent/bin; ./magentctl -start > /dev/null 2>&1" >> $RCNAME1
                    else
                        echo "su - root -c \"cd" $HOME_PATH"/MAgent/bin; ./magentctl -start > /dev/null 2>&1\"" >> $RCNAME1
                    fi
                fi
                ...
                if [ -z "$NOMNICHKLIN" -a $SYSTEMCTL -eq 0 ]; then
                    if [ $RCNAME2 = "rc.nomni2" ]; then
                        echo "echo \"$RCNAME1 execute !!\" > /tmp/rst_ckc.log" >> $RCNAME2
                        echo "source /etc/bashrc" >> $RCNAME2
                        echo "source /root/.bashrc" >> $RCNAME2
                        echo "/etc/$RCNAME1 >> /dev/null 2>&1" >> $RCNAME2
                    else
                        echo "echo \"$RCNAME1 execute !!\" > /tmp/rst_ckc_$RCTMP.log" >> $RCNAME2
                        echo "source /etc/bashrc" >> $RCNAME2
        ...
        *)
        ...
        $LINUX)
            l_nDirCheck=0
            if [ ! "$NOMNICHKLIN" -a $SYSTEMCTL -eq 0 ]; then
                CYGN="rc.cygn2"
                OMNI="rc.nomni2"
                PDCA="rc.pdca2"
                case "$RCNAME2" in
                $CYGN)
                    l_service_name="polestar-sms.service"
                    l_startname="ExecStart\=\/etc\/rc.cygn2"
                    ;;
                $OMNI)
                    l_service_name="polestar-omni.service"
                    l_startname="ExecStart\=\/etc\/rc.nomni2"
                    ;;
                $PDCA)
                    l_service_name="polestar-dca.service"
                    l_startname="ExecStart\=\/etc\/rc.pdca2"
                    ;;
                *)
                    l_nDirCheck=1
                    ;;
                ...
                if [ $l_nDirCheck -eq 0 ] ; then
                    cat $HOME_PATH/utils/INSTALL/polestar-n.service | sed -e "s/ExecStart=NULL/$l_startname/g" >  $HOME_PATH/utils/INSTALL/$l_service_name
                    if [ $UBUNTUCHK -ge 1 ]; then
                        if [ ! -d /lib/systemd/system/ ] ; then
                            l_nDirCheck=1
                        else
                            \cp -rf $HOME_PATH/utils/INSTALL/$l_service_name /lib/systemd/system/
                        fi
                    else
                        if [ ! -d /usr/lib/systemd/system/ ] ; then
                            l_nDirCheck=1
                        else
                            \cp -rf $HOME_PATH/utils/INSTALL/$l_service_name /usr/lib/systemd/system/
                        fi
                    fi
                    if [ $l_nDirCheck -eq 0 ] ; then
                        echo "systemctl"
                        systemctl daemon-reload
                        systemctl enable $l_service_name
                        systemctl status $l_service_name > /dev/null 2>&1
                        l_res_check=$?
                        if [ $l_res_check -eq 0 -o $l_res_check -eq 3 ] ; then
                            l_nDirCheck=0
                        else
                            l_nDirCheck=1
                            systemctl unmask $l_service_name
                            systemctl disable $l_service_name
                        fi
                    fi
                fi
            else
