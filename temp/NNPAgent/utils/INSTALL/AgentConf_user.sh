        ...
        *)
		...
		if [ -f /etc/$RCNAME2 ]; then
		    ...
		    $EXECUR_PATH/rm -f /etc/$RCNAME1
                ...
                if [ $RCTMP = "atint" ]; then
                    ...
                    echo "su - $USER -c \"LD_LIBRARY_PATH="$HOME_PATH"/LIB;export LD_LIBRARY_PATH;cd" $HOME_PATH"/MAgent/bin; ./magentctl -start > /dev/null 2>&1\"" >> $RCNAME1
                fi
                # add
        ...
        $HP|$OSF)
                ...
                if [ $RCTMP = "atint" ]; then
                    ...
                    #echo "cd" $HOME_PATH"/MAgent/bin; ./magentctl -start" >> $RCNAME1
                    echo "su - $USER -c \"cd" $HOME_PATH"/MAgent/bin; ./magentctl -start > /dev/null 2>&1\"" >> $RCNAME1
                fi
